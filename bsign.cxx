...
typedef struct {
  ...
  unsigned32 flags;
  unsigned64 ib;
  unsigned64 addrVirtual;
  unsigned64 addrPhysical;
  unsigned64 cbFile;
  unsigned64 cbMemory;
  unsigned64 alignment;
} PROGRAM_ELF64;
...
typedef struct {
  ...
  unsigned32 sectiontype;
  unsigned64 flags;
  unsigned64 addr;		// Virtual address during execution
  unsigned64 ib;		// Offset to section data
  unsigned64 cb;		// Length of section data
  unsigned32 iLink;		// Index of another section (link?)
  unsigned32 info;
  unsigned64 alignment;
  unsigned64 cbEntry;		// Section table entry size, if applicable
} SECTION_ELF64;
...
typedef struct {
  ...
  unsigned8 other;		// <Reserved>
  unsigned16 iSection;		// Section associated with symbol
  unsigned64 value;		// Symbol value
  unsigned64 cb;		// Size of symbol
} SYMBOL_ELF64;

		// -- FIXUP -- description of change to a file region
typedef struct _FIXUP {
  struct _FIXUP* pNext;		// Link to next fixup
  unsigned64 ib;		// Original location
  unsigned64 cb;		// Original size
  unsigned64 ibNew;		// New location
  unsigned64 cbNew;		// New size
...
size_t rewrite_elf64 (char* pb, size_t cb, int fh, FIXUP64& fixup);


void check_byte_sex (const void* pv)
{
#if defined (WORDS_BIGENDIAN)
  g_fOppositeSex = (((HDR_ELF64*)pv)->byteorder != ELF_BYTEORDER_MSB);
#else
  g_fOppositeSex = (((HDR_ELF64*)pv)->byteorder != ELF_BYTEORDER_LSB);
#endif
}
...
{
  if (cb < sizeof (HDR_ELF64))
    return false;

  check_byte_sex (pb);

  HDR_ELF64& header = *(HDR_ELF64*) pb;
  if (memcmp (header.rgbID, "\177ELF", 4) != 0
	  ...
	  && _v (header.cbEntryProgram) != sizeof (PROGRAM_ELF64))
      || _v (header.cbEntrySection) != sizeof (SECTION_ELF64)
      || _v (header.iSectionNames) >= _v (header.cEntrySection))
    return false;

  // *** FIXME: I don't recall why we need more than a header test.

  const PROGRAM_ELF64* rgProgram = (PROGRAM_ELF64*) (pb
						 + _v (header.ibHdrProgram));
  ...
  const SECTION_ELF64* rgSection = (SECTION_ELF64*) (pb
		    ...
		    size_t* pibSignature, size_t* pcbSignature)
{
  if (!is_elf (pb, cb))
    return false;
  const HDR_ELF64& hdr = *(HDR_ELF64*) pb;
  const SECTION_ELF64* rgSection
    = (SECTION_ELF64*) (pb + _v (hdr.ibHdrSection));
...
eExitStatus hash_elf (char* pb, size_t cb, int fhNew, bool fSign)
{
  ...
  FIXUP64 fixupSignature;
  memset (&fixupSignature, 0, sizeof (fixupSignature));
  fixupSignature.cb = CB_SIGNATURE;

				// Perform rewrite
  size_t cbRewrite = rewrite_elf64 (pb, cb, fhNew, fixupSignature);
...
char* new_sectionnames64 (char* pb, size_t cb,
			  SECTION_ELF64& sectionNames,
			  SECTION_ELF64& sectionSig)
{
  const HDR_ELF64& hdr = *(HDR_ELF64*) pb;

  sectionSig.ibName = sectionNames.cb;
  sectionSig.alignment = _v (unsigned64 (1));
  sectionSig.sectiontype = _v (unsigned32 (ELF_SECTION_SIGNATURE));
  unsigned64 cbSectionNamesNew = _v (sectionNames.cb)
...
void fixup_range64 (const unsigned64& ibOld, const unsigned64& cbOld,
		    unsigned64& ib, unsigned64& cb, const FIXUP64& fixup)
{
...
unsigned64 fixup_fixups64 (FIXUP64* pFixupHead)
{
  unsigned64 cb = 0;
  for (FIXUP64* pFixupOuter = pFixupHead; pFixupOuter;
       pFixupOuter = pFixupOuter->pNext) {
    pFixupOuter->ibNew = pFixupOuter->ib;
    cb += pFixupOuter->cbNew - pFixupOuter->cb;
    for (FIXUP64* pFixupInner = pFixupHead; pFixupInner;
...
void fixup_elf64_programs (const HDR_ELF64& hdr,
			   PROGRAM_ELF64* rgProgram,
			   const PROGRAM_ELF64* rgProgramOld,
			   const FIXUP64* rgFixup)
{
  for (int iProgram = 0; iProgram < _v (hdr.cEntryProgram); ++iProgram)
    for (const FIXUP64* pFixup = rgFixup; pFixup; pFixup = pFixup->pNext)
      fixup_range64 (rgProgramOld[iProgram].ib, rgProgramOld[iProgram].cbFile,
		     rgProgram[iProgram].ib, rgProgram[iProgram].cbFile,
		     *pFixup);
}

void fixup_elf64_sections (const HDR_ELF64& hdr,
			   SECTION_ELF64* rgSection,
			   const SECTION_ELF64* rgSectionOld,
			   const FIXUP64* rgFixup)
{
  for (int iSection = 0; iSection < _v (hdr.cEntrySection); ++iSection)
    for (const FIXUP64* pFixup = rgFixup; pFixup; pFixup = pFixup->pNext)
      fixup_range64 (rgSectionOld[iSection].ib, rgSectionOld[iSection].cb,
		     rgSection[iSection].ib, rgSection[iSection].cb,
		     *pFixup);
}

void fixup_elf64_header (HDR_ELF64& hdr, const HDR_ELF64& hdrOld,
			 const FIXUP64* rgFixup)
{
  for (const FIXUP64* pFixup = rgFixup; pFixup; pFixup = pFixup->pNext) {
    unsigned64 cbOld;
    ...
    if (_v (hdrOld.cEntryProgram)) {
      cbOld = _v (unsigned64 (_v (hdrOld.cbEntryProgram)
			      *_v (hdrOld.cEntryProgram)));
      cb = _v (unsigned64 (_v (hdr.cbEntryProgram)*_v (hdr.cEntryProgram)));
      fixup_range64 (hdrOld.ibHdrProgram, cbOld,
		     hdr.ibHdrProgram, cb, *pFixup);
    }

    cbOld = _v (unsigned64 (_v (hdrOld.cbEntrySection)
			    *_v (hdrOld.cEntrySection)));
    cb = _v (unsigned64 (_v (hdr.cbEntrySection)*_v (hdr.cEntrySection)));
    fixup_range64 (hdrOld.ibHdrSection, cbOld, hdr.ibHdrSection, cb, *pFixup);
  }
}

void report_elf64 (const HDR_ELF64& hdr, const SECTION_ELF64* rgSection)
{
  fprintf (stderr, "%08x %08lx header\n", 0, sizeof (hdr));
  fprintf (stderr, "%08lx %08x program header\n",
	   _v (hdr.ibHdrProgram),
	   _v (hdr.cbEntryProgram)*_v (hdr.cEntryProgram));
  fprintf (stderr, "%08lx %08x section header\n",
	   _v (hdr.ibHdrSection),
	   _v (hdr.cbEntrySection)*_v (hdr.cEntrySection));
  for (int i = 0; i < _v (hdr.cEntrySection); ++i) {
    fprintf (stderr, "%08lx %08lx section %d\n",
	     _v (rgSection[i].ib),
	     _v (rgSection[i].cb), i);
  }
}

void report_fixup (const FIXUP64* pFixup)
{
  fprintf (stderr, "%8ld %8ld->%8ld %8ld %8ld->%8ld %s\n",
	   pFixup->ib, pFixup->cb, pFixup->ib + pFixup->cb,
	   pFixup->ibNew, pFixup->cbNew, pFixup->ibNew + pFixup->cbNew,
	   pFixup->szDescription);
}

void report_fixups (const FIXUP64* pFixup)
{
...
size_t rewrite_elf64 (char* pb, size_t cb, int fh, FIXUP64& fixupSignature)
{
  const HDR_ELF64 hdrOld = *(HDR_ELF64*) pb;

  if (sizeof (hdrOld) != _v (hdrOld.cbHeader)) 		// We must have this
    return 0;
  if (_v (hdrOld.cEntryProgram)
      && sizeof (PROGRAM_ELF64)
         != _v (hdrOld.cbEntryProgram))			// We must have this
    return 0;
  if (_v (hdrOld.cEntrySection)
      && sizeof (SECTION_ELF64)
         != _v (hdrOld.cbEntrySection)) 		// We must have this
    return 0;

  const SECTION_ELF64* rgSectionOld
    = (SECTION_ELF64*) (pb + _v (hdrOld.ibHdrSection));

  FIXUP64 fixupOldSignature;
  ...
  HDR_ELF64 hdr = *(HDR_ELF64*) pb;
  const PROGRAM_ELF64* rgProgramOld
    = (PROGRAM_ELF64*) (pb + _v (hdr.ibHdrProgram));
  PROGRAM_ELF64* rgProgram
    = hdr.ibHdrProgram ? new PROGRAM_ELF64[_v (hdr.ibHdrProgram)] : NULL;
  if (rgProgram)
    memcpy (rgProgram, rgProgramOld,
	    _v (hdr.cEntryProgram)*_v (hdr.cbEntryProgram));
  SECTION_ELF64* rgSection = new SECTION_ELF64[_v (hdr.cEntrySection)
					       + (iSectionOldSignature
						  ? 0 : 1)];
  memcpy (rgSection, rgSectionOld,
	  _v (hdr.cEntrySection)*_v (hdr.cbEntrySection));

  report_elf64 (hdr, rgSectionOld);

  SECTION_ELF64& sectionSig   = rgSection[_v (hdr.cEntrySection)];
  SECTION_ELF64& sectionNames = rgSection[iSectionOldSignature
					  ? iSectionOldSignature
					  : _v (hdr.iSectionNames)];
  memset (&sectionSig, 0, sizeof (sectionSig));

  char* rgbSectionNames
    = new_sectionnames64 (pb, cb, sectionNames, sectionSig);
  if (!iSectionOldSignature)
    hdr.cEntrySection = _v (_v (hdr.cEntrySection) + (unsigned32) 1);

				// Generate fixup lists
  FIXUP64* pFixup	= new FIXUP64;
  ...
  if (fixupOldSignature.cb) {
    pFixup		= new FIXUP64;
    memcpy (pFixup, &fixupOldSignature, sizeof (FIXUP64));
  ...
  printf ("@perform fixups\n"); report_fixups (pFixupHead);
				...
				// Perform fixups
  unsigned64 cbNew = fixup_fixups64 (pFixupHead) + cb;
  fixup_elf64_header (hdr, hdrOld, pFixupHead);
  if (_v (hdr.cEntryProgram))
    fixup_elf64_programs (hdrOld, rgProgram, rgProgramOld, pFixupHead);
  fixup_elf64_sections (hdrOld, rgSection, rgSectionOld, pFixupHead);
